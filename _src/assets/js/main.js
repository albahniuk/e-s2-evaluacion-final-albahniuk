...
function writeShow(e) {
  ...
  if (keycode == 13) {
    ...
    fetch(urlApi + show)
      ...
      .then(data => {
        if (data.length === 0) {
          alert('La búsqueda no corresponde a ninguna serie');
        }
        for(let i=0; i<data.length; i++) {
          ...
          if(favoritesLocalStorage !== null) {
            ...
            //comprobar si el id de la serie está en favoritos (devuelve true/false) y pasarlo a string válida (si el findIndex no encuentra nada devuelve -1)
...
inputUser.addEventListener('keyup', writeShow);
